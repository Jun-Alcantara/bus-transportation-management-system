...
export default function EditRoute({ route, districts, activeSchoolYear, activeSchoolTerm }) {
    ...
    const { data, setData, put, processing } = useForm({
        ...
        date_activated: route.date_activated || '',
        date_deactivated: route.date_deactivated || '',
        runs: route.runs?.map(run => ({
            id: run.id,
            run_code: run.run_code,
            status: run.status,
            date_activated: run.date_activated || '',
            date_deactivated: run.date_deactivated || ''
        })) || [{ 
            run_code: '', 
            status: 'active', 
            date_activated: '',
    ...
    return (
        <DashboardLayout>
            <div className="max-w-6xl mx-auto space-y-8">
                {/* Page Header */}
                <div>
                    <h2 className="text-3xl font-bold">Edit Route</h2>
                    <p className="text-muted-foreground mt-2">
                        Update route information and manage associated runs
                    </p>
                </div>
                ...
                <Card className="bg-blue-50 border-blue-200">
                    <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="font-medium text-blue-900">Route School Period</h3>
                                <p className="text-blue-700 text-sm">
                                    {route.school_year?.name} - {route.school_term?.name}
                                </p>
                            </div>
                            <div className="text-xs text-blue-600">
                                This route belongs to this school period
                            </div>
                        </div>
                    </CardContent>
                </Card>
                                    ...
                                    <div key={index} className="border border-gray-200 rounded-lg p-6 space-y-4">
                                        <div className="flex items-center justify-between">
                                            <h4 className="font-medium">
                                                Run {index + 1} {run.id && <span className="text-sm text-muted-foreground">(ID: {run.id})</span>}
                                            </h4>
                                            {data.runs.length > 1 && (
                                                <Button
                                                    type="button"
